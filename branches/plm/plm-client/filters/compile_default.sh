...
echo
echo Welcome to the [ Compile Default ] Filter V1.1
echo
...
# Compile the kernel 
compile_kernel () {
  make dep bzImage &> $COMPILE_LOG || touch $HOME/ERROR
  if [ `grep -c "CONFIG_MODULES=y" .config` != 0 ] && [ ! -f $HOME/ERROR ]; then
      make modules &> $COMPILE_LOG || touch $HOME/ERROR
  fi
}
