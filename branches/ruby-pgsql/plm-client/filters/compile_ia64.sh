...
echo
echo Welcome to the [ Default ia64 Compile ] Filter V1.1
echo
echo This filter compiles the kernel source twice, once
echo for UP systems and once for SMP systems.  All the other
...
# Compile the kernel 
compile_kernel () {
  make dep vmlinux modules &> $COMPILE_LOG || touch $HOME/ERROR
}
...
#
# Test 2: Does the vmlinux file exist, why is vmlinux not put in
#         arch/ia64/boot?
#
test_2 () {
  if [ ! -f vmlinux ]; then
    log RESULT: FAIL
    log RESULT-DETAIL: Missing vmlinux file after build
    dump_log
  fi
}
...
log RESULT: PASS
log "RESULT-DETAIL: Compiles OK for UP & SMP with default options"
