...
if [ $HAS_ALLNOCONFIG == 1 ]; then
	...
	(make mrproper > /dev/null 2>&1)
	(make $MAKECONFIGOPT allnoconfig > $KERNEL_ALLNO_OUTPUT 2>&1)
        no_modversions
	alt_configs
	if [ "$BASE_VERSION" == 2.4 ]; then 
...
cp $TMPFILE $WARN_LIST
rm $TMPFILE
echo

#
# For the final error count and warning count, do not use the
# accumulated totals, but rather the sorted and uniqed totals.
#
