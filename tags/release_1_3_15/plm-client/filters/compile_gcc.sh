...
echo
echo Welcome to the [ gcc Compile ] Filter v1.0
echo
echo This filter compiles the gcc and runs make check.
echo
...
# Compile the kernel 
compile_gcc () {
  make bootstrap &> $COMPILE_LOG || touch $HOME/ERROR
}
...
# 
# Test 1: Does the gcc directory exist
#
test_1 () {
  if [ ! -d gcc ]; then
    log RESULT: FAIL
    log "RESULT-DETAIL: Missing gcc directory (internal error?)"
    exit 0
  fi
}
...
#
test_3 () {
  make -k check &> $CHECK_LOG || touch $HOME/ERROR
}
